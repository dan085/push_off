...
create_json(List1, List2) ->
  lists:append(["{\"aps\":{", create_keyvalue(List1), ", \"category\": \"QuickReply\", \"mutable-content\": 1, \"content-available\": 1 }, ", create_keyvalue(List2), "}"]).

create_json_alert(List1, List2) ->
  lists:append(["{\"aps\":{ \"alert\" : {", create_keyvalue(List1), ", \"category\": \"QuickReply\" } , \"category\": \"QuickReply\" , \"mutable-content\": 1, \"content-available\": 1 }, ", create_keyvalue(List2), "}"]).
        ...
        Socket ->
            ...
            case NewPendingList of
                ...
                _ ->
                    ...
                    Body = proplists:get_value(body, NewPendingList),
                    From = proplists:get_value(from, NewPendingList),
                    Subject = proplists:get_value(subject, NewPendingList),
                    Nicky_name = proplists:get_value(thread, NewPendingList),
                  

                     if 
                        Subject == "0" -> 

                            Msg = [{title, From}, {body, Body}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
                            ...
                            fun({MessageId, {_, Payload, Token, _}}, Acc) ->
                                                    ...
                                                    PayloadBin = list_to_binary(EncodedMessage),
                        ...
                        Subject == "8" -> 
                            ...
                            Msg = [{title, From}, {'loc-key', "GROUPS_LINKS"}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
                        ...
                        Subject == "9" -> 
                            ...
                            Msg = [{title, From}, {'loc-key', "PICTURE_NOTIFY"}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
                        ...
                        Subject == "10" -> 
                            ...
                            Msg = [{title, From}, {'loc-key', "VOICE_NOTIFY"}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
                        ...
                        Subject == "11" -> 
                            ...
                            Msg = [{title, From}, {'loc-key', "VIDEO_NOTIFY"}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
                        ...
                        Subject == "12" -> 
                            ...
                            Msg = [{title, From}, {'loc-key', "DOCUMENTS_NOTIFY"}],
                            Args = [{source, From}, {subject, Subject}, {nickname, Nicky_name}],
                            JSON = create_json_alert(Msg, Args),
                            EncodedMessage = jiffy:encode(JSON),
