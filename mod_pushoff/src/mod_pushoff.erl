...
stanza_to_payload(#message{body = Body, type = Type, subject = Subject,from=From}) -> [{body, Body},{type,Type},{subject,Subject},{from,From}];
stanza_to_payload(_) -> [].
...
offline_message({_, #message{to = To} = Stanza} = Acc) ->
    ...
    ?INFO_MSG("++++++++ mod_pushoff:start(~p, ~p): workers ~p", Payload),
